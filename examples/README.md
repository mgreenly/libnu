...
```bash
gcc -std=c17 -Wall -Wextra -g <example>.c $(pkg-config --cflags --libs nulib) -o <example>
./<example>
```

Or with static linking:
```bash
gcc -std=c17 -Wall -Wextra -g -static <example>.c $(pkg-config --cflags --libs --static nulib) -o <example>
./<example>
```
...
```bash
pkg-config --exists nulib && echo "Found" || echo "Not found"
pkg-config --modversion nulib
pkg-config --cflags --libs nulib
```
